...
```{r}
df=readRDS("group5AA_Black-Boopathy_train_11_27.rds")
names(df) <- gsub(" ", "_", names(df))
names(df) <- gsub("-", "_", names(df))  
names(df) <- gsub("\\+", "plus", names(df))

saveRDS(df,"group5AA_Black-Boopathy_train_11_27.rds")

holdout=readRDS("holdout_df.rds")

names(holdout) <- gsub(" ", "_", names(holdout))
names(holdout) <- gsub("-", "_", names(holdout))
names(holdout) <- gsub("\\+", "plus", names(holdout))
...
```{r}
saveRDS(logit_fit,"logit_model.rds")
saveRDS(pred.test,"pred_test.rds")
saveRDS(df.us,"df_us.rds")
